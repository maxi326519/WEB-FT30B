...
function buildToDo(todo, index) {
  // Tu código acá:
  let toDoShell = document.createElement('div');    // Creamos un nuevo div
  let toDoText = document.createElement('span');    // Creamos un nuevo span

  toDoShell.setAttribute('class', 'toDoShell');       // Agregamos un nuevo atributo class al div

  toDoText.innerHTML = todo.description;            // Cambiamos el texto dentro del span
  toDoText.setAttribute('id', `${index}`);

  if(todo.description === true){
    toDoText.setAttribute('class', 'completeText');      // Si se completo la tarea se le agrega un nuevi atributo
  }

  toDoShell.appendChild(toDoText);                  // Por ultimo agregamos el span een el div
...
function buildToDos(toDos) {
  // Tu código acá:
  let arrToDos =  toDos.map(function(object, index){
...
function displayToDos() {
  ...
  let newArr = buildToDo(toDoItems);
